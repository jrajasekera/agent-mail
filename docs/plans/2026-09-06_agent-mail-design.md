...
| Codex CLI + Desktop | `codex queue --thread <thread> --message "<metadata header>"` — a write to `~/.codex/queue_1.sqlite`; arrives as a user message and **wakes an idle session into an unprompted turn** (verified on codex-cli 0.153.4, 2026-09-07). `codex queue` **is durable across a stopped process** (verified 2026-09-07, `amail-a1w`): queueing to a known thread whose process has exited exits 0, persists a `queued_items` row, and the row is drained into the transcript as user input when the thread is later resumed. The exit-1 path is *unknown thread id only* (`no rollout found for thread id ...`), not "no live process". **`amail send` does not currently use this for offline targets** — `mail.send` filters `status == "offline"` out of the push list (`mail.py:107`), so offline Codex mail waits for the SessionStart backstop instead. Whether to push to offline Codex threads is an open design question (`amail-8ny` retest notes). Bounded subprocess timeout on the sender side. A Codex session still has no thread id at all until its first turn (`amail-eec`) |
